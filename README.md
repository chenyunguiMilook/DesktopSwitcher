...
| 点击最右侧 `中` / `A` | 中英文输入法互切 |
| 按住任意位置拖动 | 移动组件，自动记住位置 |

`⌃` 点击等同右键。橙色小圆点表示尚未获得辅助功能权限；授权后消失。

**拖动从任意位置都可以**，包括从按钮上按下 —— 指针移动超过 4pt 就变成拖动，没移动才算点击。
如果拖不动，检查右键菜单里的「锁定位置」是不是开着。

> 实现上这需要 AppKit 接管整个鼠标手势。`NSHostingView` 会吞掉 mouse-down，
> 导致 `isMovableByWindowBackground` 永远等不到未处理的事件，组件**完全无法拖动**。
> 又因为胶囊本身就很小，没法从中划出一块拖拽把手，所以改由 AppKit 判定"按下后是否移动"，
> 并自行派发按钮点击（`SwitcherMetrics` 同时驱动 SwiftUI 布局和命中测试，两边不会走偏）。
> 悬停高亮仍由 SwiftUI 负责，它只需要 mouse-moved 事件。
...
多显示器下每块屏各有一条空间栏。缩略图**自己的**坐标没法用来归属显示器
（折叠状态下恒为 y = -32，在屏幕外），但**父级空间栏**的坐标是干净的：

```
bar[0] pos=(0,0)      4 个缩略图  → 主屏 (0,0,3008×1269)
bar[1] pos=(620,1269) 2 个缩略图  → 屏幕 (620,-900,1440×900)
```

所以代码按空间栏分组、用栏的位置匹配显示器，再在组内按索引取缩略图。
早期版本把所有显示器的缩略图拍平成一个列表按索引点，接上第二块屏就会点错桌面。
...
**2. `open -g` 会让 Mission Control 自己关掉。**
`-g`（不切到前台）在单显示器本机上能用，但接上多块屏后实测：Mission Control 打开约 300ms
就自行消失，任何按下都来不及落地，表现为"闪一下但不切换"。去掉 `-g` 让它正常获得焦点后，
实测可稳定保持 3.9 秒以上。

**3. 不能每轮轮询都请求打开 Mission Control。**
...
- 拖动与点击：从进程内合成真实鼠标事件验证 —— 按住按钮拖动使窗口从 (909,24) 移到 (789,114)
  并正确还原；原地按下松开则触发切换（space 4 → 3）
- 多显示器：3 块屏、2 条空间栏环境下，按空间栏分组后点击命中的是面板所在那块屏的桌面
